...
// Multiple Character
unsigned char   keyCode1[] = {0x0e, 0x08, 0x1c, 0x05, 0x12, 0x04, 0x15, 0x07}; // Set "keyboard"
...
void Keyboard(void)
{
    ...
    if(!HIDTxHandleBusy(lastINTransmission))
    {
        ...
        if(remainedSendChar)
        {
            // Send remained chatacter
            hid_report_in[0] = 0;
            hid_report_in[1] = 0;
            hid_report_in[2] = keyCode1[numberOfChar1 - remainedSendChar];
            hid_report_in[3] = 0;
            hid_report_in[4] = 0;
            hid_report_in[5] = 0;
            hid_report_in[6] = 0;
            ...
            if(Switch1IsPressed())
            {
                ...
                // Start sending multiple charaters
                remainedSendChar = numberOfChar1;
                hid_report_in[0] = 0;
                hid_report_in[1] = 0;
                hid_report_in[2] = keyCode1[numberOfChar1 - remainedSendChar];
                hid_report_in[3] = 0;
